...
Let's try to use the cut-up technique on the top popular hymn, *Se, nu stiger solen af havets skød*, by Jacob Knudsen, 1891. This is number [754](link:https://www.dendanskesalmebogonline.dk/salme/754) in *Den Danske Salmebog*.

```{r}
hymns %>% 
  filter(doc_id == 754) %>% 
  select(doc_id, verse, text) 
```
...
```{r}
cut754 <- annotated_hymns %>% cut_up(754, except = "PUNCT")

cut754 %>% select(doc_id, token, upos, vowels, token_new)
```
...
```{r}
bind_cols(
  collapse_annotation(cut754, token), 
  collapse_annotation(cut754, token_new)
  ) %>% 
  select(doc_id = doc_id...1, 
